...
* Here are some other ways a player may interact with their list of cards:
  ...
  - search: Let's say Alice starts the game - one of the first things she needs to do is to check if her first card (h 3) is in Bob's card list.  So Bob, at this moment, should call a function to search this card (h 3) in his list of cards. Obiviously, (h 3) is not in Bob's list. So Alice goes to the next card (s 2) and let Bob search for (s 2). Still failed. ... (many failures here) Until Alice lets Bob search for (c 3), Bob finds it!
  - delete: Obiviously, Alice and Bob should throw their (c 3) away. Both of them should call a function to delete the card (c 3) in their lists of cards -- what should be the parameter to delete?

## Overload operator "<<"

Overload operator "<<" is a useful feature in C++. It enables objects of various classes to customize their stream extraction behavior. ("<<" is for output. ">>" is for input with similar usage but not discussed here.)
To specificly illuatrate how to overload operator "<<", here is one example.
...
A class Person is defined, and it has a overload "<<" function as a friend non-member function.
Inside this operator function, people can freely specify what information would be printed out when operator "<<" is applied to this class, Person. Here we want it to print its name. 

With this overload function, we can directly use it on Person like how we print those built-in data types i.e. int, double.
